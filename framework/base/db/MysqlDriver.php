...
class MysqlDriver implements DbInterface {
    ...
    public function select($table, array $condition = array(), $field = '*', $order = NULL, $limit = NULL, $group = NULL) {
    ...
    public function insert_batch($table, array $setData = array(), $escape = "", $batch_size = 100) {

        $table = $this->_table($table);

        if ($setData === NULL || empty($setData)) {
            throw new \Exception('Database SQL: "' . $this->getSql() . '". ErrorInfo: ' . "", 500);
        }
        //获取键值
        $key = array_keys(reset($setData));


        // Batch this baby
        foreach ($setData as $set) {
            $affected_array[] = "(" . implode(",", $set) . ")";
        }
//
        $status = $this->execute("INSERT INTO {$table} (" . implode(', ', $key) . ") VALUES " . implode(', ', $affected_array));
        return $status;
    }
    ...
    public function count($table, array $condition = array(), $distinct = '') {
        $table = $this->_table($table);
        $condition = $this->_where($condition);
        $field = $distinct == '' ? '*' : $distinct;
        $count = $this->query("SELECT COUNT(" . $field . ") AS __total FROM {$table} " . $condition['_where'], $condition['_bindParams']);
        return isset($count[0]['__total']) && $count[0]['__total'] ? $count[0]['__total'] : 0;
    }

    public function getCount($table, array $condition = array(), $distinct = '', $group = '') {
        $table = $this->_table($table);
        $group = !empty($group) ? ' GROUP BY ' . $group : '';
        $condition = $this->_where($condition);
        $field = $distinct == '' ? '*' : $distinct;
        $count = $this->query("SELECT COUNT(" . $field . ") AS __total FROM {$table} {$condition['_where']} $group ", $condition['_bindParams']);
