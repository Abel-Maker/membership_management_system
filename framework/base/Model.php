...
class Model {
    ...
    /**
     *
     * @param string $distinct , add by jlp，区分需要根据distinct来计算数量的
     * @return mixed
     */
    public function select($distinct = '') {
        ...
        if (!empty($this->options['pager'])) {
            ...
            $count = $this->getDb()->getCount($table, $where, $distinct, $group);
        ...
        return $this->getDb()->select($table, $where, $field, $order, $limit, $group);
    }
    ...
    public function insert_batch() {
        ...
        return $this->getDb()->insert_batch($this->_getTable(), $this->_getData());
    }
    ...
    public function count($distinct = '') {
        return $this->getDb()->count($this->_getTable(), $this->_getWhere(), $distinct);
    }
