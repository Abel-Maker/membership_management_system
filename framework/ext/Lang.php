...
class Lang {
    ...
    static public function init($config = array()) {
        ...
        self::$config['LANG_PACK_PATH'] = isset($config['LANG_PACK_PATH']) ? $config['LANG_PACK_PATH'] : LANG_PATH;
    ...
    static public function get($key, $pack = '') {
        ...
        //改进可以读取menu.home这样的类型
        if (strpos($key, '.')) {
            list($key1, $key2) = explode('.', $key, 2);
            return isset($lang_array[$key1][$key2]) ? $lang_array[$key1][$key2] : $key;
        } else {
