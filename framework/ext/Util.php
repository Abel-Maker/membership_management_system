...
class Util {
    ...
    public static function miEncode($data, $key = '', $expire = 0) {
    ...
    //miEncode之后的解密函数，$string待解密的字符串，$key，密钥
    public static function miDecode($string, $key = '') {
        $ckey_length = 4;
        $key = md5($key);
        $keya = md5(substr($key, 0, 16));
        $keyb = md5(substr($key, 16, 16));
    ...
    public static function encrypt($str, $key = '') {
        ...
        for ($i = 0, $count = strlen($str); $i < $count; $i += $len) {
            $coded .= substr($str, $i, $len) ^ $key;
        }
